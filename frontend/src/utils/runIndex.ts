 ...
 * benchmark is a flat data/{id}.json.zst document, so the glob lists the files and the basename is the
...
const modules = import.meta.glob('/data/*.json.zst', {
  query: '?url',
  import: 'default',
  eager: true,
...
export function loadRunIndex(): RunIndexEntry[] {
  return Object.entries(modules)
    .map(([path, url]) => ({ id: path.replace(/^.*\/data\//, '').replace(/\.json\.zst$/, ''), url }))
